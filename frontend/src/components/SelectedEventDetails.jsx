...
const SelectedEventDetails = ({ events = [], selectedDate = "", onEdit = () => {} }) => {
    ...
    if (!Array.isArray(events)) {
        ...
        return <p className="text-muted-foreground text-sm italic">No events found.</p>;
    }
    ...
    return (
        <div className="mt-6">
            ...
            {/* Render each event */}
            {events.map(event => (
                <div key={event.event_id} className="bg-card border border-border p-4 rounded-lg shadow-md mt-4 flex justify-between items-center">
                    <div className="text-left">
                        ...
                        <p className="text-muted-foreground mt-2">{event.notes}</p>
                    </div>
